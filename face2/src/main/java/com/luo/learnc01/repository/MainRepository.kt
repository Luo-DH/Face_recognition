package com.luo.learnc01.repository
...
import com.luo.learnc01.face.MTCNN
import com.luo.learnc01.face.RetinaFace
...
//            this["FF"] = Utils.readFromAssets(context, "FF.jpg")!!
...
//            this["66666"] = Utils.readFromAssets(context, "66666.jpg")!!
    ...
    fun detectDataBaseFace(bitmap: Bitmap) =
        MTCNN().detect(bitmap, bitmap.width, bitmap.height).let { box ->
            box.maxByOrNull{ it.area }
        }

    fun detectDataBaseFaceRetina(bitmap: Bitmap) =
        RetinaFace().detect3(bitmap, 1f).let { box ->
            box.maxByOrNull{ it.area }
        }
    ...
    fun getDataBaseFeatureAndBitmap(bitmap: Bitmap, landmarks: IntArray) =
        DBMsg(
//            ArcFace().getFeature(bitmap),
            ArcFace().getFeatureWithWrap(bitmap, landmarks),
            bitmap
        )
