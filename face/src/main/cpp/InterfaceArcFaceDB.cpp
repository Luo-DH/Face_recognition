...
Java_com_luo_face_ArcFaceDB_init(JNIEnv *env, jobject thiz, jobject manager) {
...
Java_com_luo_face_ArcFaceDB_getFeature(JNIEnv *env, jobject thiz, jobject bitmap) {
...
Java_com_luo_face_ArcFaceDB_calCosineDistance(JNIEnv *env, jobject thiz,
...
void getAffineMatrixDB(float *src_5pts, const float *dst_5pts, float *M) {
    float src[10], dst[10];
...
void warpAffineMatrixDB(ncnn::Mat src, ncnn::Mat &dst, float *M, int dst_w, int dst_h) {
    int src_w = src.w;
    int src_h = src.h;
...
ncnn::Mat preprocessDB(ncnn::Mat img, int *info) {
    ...
    float M[6];
    getAffineMatrixDB(src, dst, M);
    ncnn::Mat out;
    warpAffineMatrixDB(img, out, M, image_w, image_h);
    return out;
}
...
extern "C"
JNIEXPORT jfloatArray JNICALL
Java_com_luo_face_ArcFaceDB_getFeatureWithWrap(JNIEnv *env, jobject thiz, jobject bitmap,
                                               jintArray landmarks) {
    ...
    // 人脸对齐
    ncnn::Mat inputImg = preprocessDB(in, mtcnnLandmarks);
...
extern "C"
JNIEXPORT jfloatArray JNICALL
Java_com_luo_face_ArcFaceDB_getFeatureWithWrap2(JNIEnv *env, jobject thiz,
    ...
    // 人脸对齐
    ncnn::Mat inputImg = preprocessDB(ncnn_img_db, mtcnnLandmarks);
...
Java_com_luo_face_ArcFaceDB_compareFeature(JNIEnv *env_db, jobject thiz_db,
